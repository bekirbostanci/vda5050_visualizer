...
const connectionType = ref(loadSavedConfig().connectionType || (typeof window.electron !== 'undefined' ? "mqtt" : "websocket"));
const version = ref(0);
...
export function useVDA5050() {
    ...
    watch(() => vda5050Visualizer?.robotList.value, (newRobots) => {
        if (newRobots) {
            ...
            if (!agvControllers.value.has(key)) {
                ...
                if (connectionType.value === 'mqtt' && isElectronAvailable) {
                    window.electron.ipcRenderer.on("mqtt-message", (data: any) => {
                        ...
                        if (topic.includes(robot.serialNumber) && topic.includes(robot.manufacturer)) {
    ...
    function extractAgvIdFromTopic(topic: string): AgvId | null {
        ...
        return parts.length >= 4 ? { manufacturer: parts[2], serialNumber: parts[3] } : null;
    }

    function robotExists(agvId: AgvId): boolean {
        return vda5050Visualizer?.robotList.value.some(
            (robot) => robot.serialNumber === agvId.serialNumber && robot.manufacturer === agvId.manufacturer
        ) || false;
    }
