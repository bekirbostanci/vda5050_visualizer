...
const publishOrder = () => {
  try {
    ...
    if (!orderForm.value.headerId || !orderForm.value.manufacturer || !orderForm.value.serialNumber || !orderForm.value.version || !orderForm.value.orderId || orderForm.value.orderUpdateId === undefined) {
      error.value = 'Missing required fields: headerId, manufacturer, serialNumber, version, orderId, orderUpdateId';
      return;
    }
  ...
  <div class="space-y-4">
    ...
    <div class="space-y-4">
      ...
      <div class="border rounded-lg p-4 space-y-3">
        <div class="flex items-center justify-between cursor-pointer" @click="toggleSection('order-basic')">
          <h3 class="font-semibold text-sm">Basic Information</h3>
          <Icon 
            :icon="expandedSections.has('order-basic') ? 'material-symbols:expand-less' : 'material-symbols:expand-more'" 
            class="w-4 h-4" 
          />
        </div>
        <div v-if="expandedSections.has('order-basic')" class="space-y-3 pt-2">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input v-model.number="orderForm.headerId" type="number" class="h-8 text-xs" />
            </div>
            <div>
              <Label class="text-xs">Order Update ID</Label>
              <Input v-model.number="orderForm.orderUpdateId" type="number" class="h-8 text-xs" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="flex gap-2">
              <Input 
                :model-value="orderForm.timestamp ? new Date(orderForm.timestamp).toISOString().slice(0, 16) : ''"
                @update:model-value="(val) => orderForm.timestamp = val ? new Date(val).toISOString() : new Date().toISOString()"
                type="datetime-local" 
                class="h-8 text-xs flex-1" 
              />
              <Button size="sm" variant="outline" class="h-8 text-xs" @click="orderForm.timestamp = new Date().toISOString()">
                Now
              </Button>
            </div>
          </div>
      ...
      <div class="border rounded-lg p-4 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center justify-between cursor-pointer flex-1" @click="toggleSection('order-nodes')">
            <h3 class="font-semibold text-sm">Nodes ({{ orderForm.nodes.length }})</h3>
            <Icon 
              :icon="expandedSections.has('order-nodes') ? 'material-symbols:expand-less' : 'material-symbols:expand-more'" 
              class="w-4 h-4" 
            />
          </div>
          <Button size="sm" variant="outline" class="h-7 text-xs ml-2" @click="addNode">
            <Icon icon="material-symbols:add" class="w-3 h-3 mr-1" />
            Add Node
          </Button>
        </div>
        <div v-if="expandedSections.has('order-nodes')" class="space-y-3 pt-2">
          <div v-for="(node, nodeIndex) in orderForm.nodes" :key="nodeIndex" class="border rounded p-3 space-y-2 bg-muted/30">
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium">Node {{ nodeIndex + 1 }}</span>
              <Button size="sm" variant="ghost" class="h-6 w-6 p-0" @click="removeNode(nodeIndex)">
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <Input v-model.number="node.sequenceId" type="number" class="h-7 text-xs" />
              </div>
            </div>
            <div class="flex items-center gap-2">
              <input type="checkbox" v-model="node.released" :id="'node-released-' + nodeIndex" class="w-4 h-4" />
              <Label :for="'node-released-' + nodeIndex" class="text-xs cursor-pointer">Released</Label>
            </div>
            <div v-if="!node.nodePosition" class="pt-1">
              <Button size="sm" variant="outline" class="h-6 text-xs" @click="node.nodePosition = { x: 0, y: 0, mapId: '' }">
                <Icon icon="material-symbols:add-location" class="w-3 h-3 mr-1" />
                Add Position
              </Button>
            </div>
            <div v-else class="grid grid-cols-3 gap-2">
              <div>
                ...
                <Input v-model.number="node.nodePosition.x" type="number" class="h-7 text-xs" />
              </div>
              <div>
                <Label class="text-xs">Y</Label>
                <Input v-model.number="node.nodePosition.y" type="number" class="h-7 text-xs" />
              </div>
              <div class="space-y-1">
                <Label class="text-xs">Map ID</Label>
                <div class="flex gap-1">
                  <Input v-model="node.nodePosition.mapId" class="h-7 text-xs flex-1" />
                  <Button size="sm" variant="ghost" class="h-7 w-7 p-0" @click="node.nodePosition = undefined">
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <Label class="text-xs">Actions ({{ node.actions?.length || 0 }})</Label>
                <Button size="sm" variant="ghost" class="h-6 text-xs" @click="addActionToNode(nodeIndex)">
                  <Icon icon="material-symbols:add" class="w-3 h-3 mr-1" />
                  Add Action
                </Button>
              </div>
              <div v-for="(action, actionIndex) in node.actions" :key="actionIndex" class="pl-2 border-l-2 space-y-2 bg-background/50 p-2 rounded">
                <div class="grid grid-cols-2 gap-2">
                  <div>
                    <Label class="text-xs">Action ID</Label>
                    <Input v-model="action.actionId" placeholder="Action ID" class="h-6 text-xs" />
                  </div>
                  <div>
                ...
                <div v-if="getActionDescription(action.actionType)" class="text-xs text-muted-foreground italic">
                  {{ getActionDescription(action.actionType) }}
                </div>
                <div>
                  <Label class="text-xs">Blocking Type</Label>
                  <select v-model="action.blockingType" class="h-6 text-xs w-full rounded border px-2 bg-background">
                ...
                <div class="space-y-1">
                  ...
                  <Textarea v-model="action.actionDescription" class="h-12 text-xs" placeholder="Additional information on the action" />
                </div>
                <div class="space-y-2">
                  <div class="flex items-center justify-between">
                ...
                <Button size="sm" variant="ghost" class="h-5 text-xs" @click="removeActionFromNode(nodeIndex, actionIndex)">
                  Remove Action
                </Button>
              </div>
            </div>
      ...
      <div class="border rounded-lg p-4 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center justify-between cursor-pointer flex-1" @click="toggleSection('order-edges')">
            <h3 class="font-semibold text-sm">Edges ({{ orderForm.edges.length }})</h3>
            <Icon 
              :icon="expandedSections.has('order-edges') ? 'material-symbols:expand-less' : 'material-symbols:expand-more'" 
              class="w-4 h-4" 
            />
          </div>
          <Button size="sm" variant="outline" class="h-7 text-xs ml-2" @click="addEdge">
            <Icon icon="material-symbols:add" class="w-3 h-3 mr-1" />
            Add Edge
          </Button>
        </div>
        <div v-if="expandedSections.has('order-edges')" class="space-y-3 pt-2">
          <div v-for="(edge, edgeIndex) in orderForm.edges" :key="edgeIndex" class="border rounded p-3 space-y-2 bg-muted/30">
            <div class="flex items-center justify-between">
              <span class="text-xs font-medium">Edge {{ edgeIndex + 1 }}</span>
              <Button size="sm" variant="ghost" class="h-6 w-6 p-0" @click="removeEdge(edgeIndex)">
                <Icon icon="material-symbols:delete" class="w-3 h-3" />
              </Button>
            </div>
            <div class="grid grid-cols-2 gap-2">
              ...
              <div>
                <Label class="text-xs">Sequence ID</Label>
                <Input v-model.number="edge.sequenceId" type="number" class="h-7 text-xs" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-2">
              <div>
            ...
            <div class="flex items-center gap-2">
              <input type="checkbox" v-model="edge.released" :id="'edge-released-' + edgeIndex" class="w-4 h-4" />
              <Label :for="'edge-released-' + edgeIndex" class="text-xs cursor-pointer">Released</Label>
            </div>
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                <Label class="text-xs">Actions ({{ edge.actions?.length || 0 }})</Label>
                <Button size="sm" variant="ghost" class="h-6 text-xs" @click="addActionToEdge(edgeIndex)">
                  <Icon icon="material-symbols:add" class="w-3 h-3 mr-1" />
                  Add Action
                </Button>
              </div>
              <div v-for="(action, actionIndex) in edge.actions" :key="actionIndex" class="pl-2 border-l-2 space-y-2 bg-background/50 p-2 rounded">
                <div class="grid grid-cols-2 gap-2">
                  <div>
                    <Label class="text-xs">Action ID</Label>
                    <Input v-model="action.actionId" placeholder="Action ID" class="h-6 text-xs" />
                ...
                <div v-if="getActionDescription(action.actionType)" class="text-xs text-muted-foreground italic">
                  {{ getActionDescription(action.actionType) }}
                    ...
                    <Button size="sm" variant="ghost" class="h-5 text-xs" @click="removeActionFromEdge(edgeIndex, actionIndex)">
                      Remove Action
                    </Button>
              </div>
            </div>
